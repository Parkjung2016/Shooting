...
public class GameManager : Singleton<GameManager>
{
    ...
    [SerializeField]
    private string[] SoliloquyText;
    [SerializeField]
    private string[] Soliloquy2Text;
    ...
    [SerializeField]
    private GameObject[] _metroSkillPrefabs;
    private Fox _fox;
    private ARC _arc;

    [HideInInspector]
    public PlayerControllerBase _pC;
    private AudioSource _audioSource;
    private AudioSource _bgmAudioSource;

    [SerializeField]
    private AudioClip[] _skillClips;
    ...
    protected override void Awake()
    {
        ...
        Instantiate(_playerPrefabs[PlayerDataManager.Instance.PlayerData.PlayerType], _startPlayerPos.position, Quaternion.identity);
    ...
    private void CreateUIManager()
    {
        ...
        UIManager.Instance.Init(FindAnyObjectByType<Canvas>().transform, _playerSprites, _playerImgPos, _playerImgScale);
        if(PlayerDataManager.Instance.PlayerData.PlayerType==0)
                ...
                switch (_skillType)
                {
                    case SkillType.METEOR:
                        for (int i = 0; i < 18; i++)
                        {
                            ...
                            Meteor obj = Instantiate(_metroSkillPrefabs[UnityEngine.Random.Range(0, _metroSkillPrefabs.Length)], Vector3.zero, Quaternion.identity).GetComponent<Meteor>();
                            obj._speed = UnityEngine.Random.Range(6f, 10f);
                            if (dir == 0)
                            {

                                obj.transform.position = new Vector3(UnityEngine.Random.Range(-12.3f, -9.2f), UnityEngine.Random.Range(5, 12f));
                            }
                            else
                            {
                                obj.transform.position = new Vector3(UnityEngine.Random.Range(9.2f, 12.3f), UnityEngine.Random.Range(5, 12f));
